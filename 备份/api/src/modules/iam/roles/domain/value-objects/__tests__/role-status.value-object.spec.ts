import { RoleStatus, RoleStatusValue } from '../role-status.value-object'
...
describe('RoleStatusValue', () => {
  ...
  describe('validation', () => {
    it('应该拒绝无效的状态值', () => {
      expect(() => new RoleStatusValue('invalid' as RoleStatus)).toThrow(
        '无效的角色状态',
      )
    })

    it('应该拒绝null值', () => {
      expect(() => new RoleStatusValue(null as any)).toThrow(
        '角色状态不能为空, 角色状态必须是字符串',
      )
    })

    it('应该拒绝undefined值', () => {
      expect(() => new RoleStatusValue(undefined as any)).toThrow(
        '角色状态不能为空, 角色状态必须是字符串',
      )
    })
  })
  ...
  describe('methods', () => {
    ...
    it('getDescription应该返回描述', () => {
      const status = new RoleStatusValue(RoleStatus.ACTIVE)
      expect(status.getDescription()).toBe(
        '角色处于激活状态，可以正常分配给用户',
      )
    })
  ...
  describe('display names', () => {
    it('应该返回正确的显示名称', () => {
      expect(new RoleStatusValue(RoleStatus.ACTIVE).getDisplayName()).toBe(
        '激活',
      )
      expect(new RoleStatusValue(RoleStatus.SUSPENDED).getDisplayName()).toBe(
        '禁用',
      )
      expect(new RoleStatusValue(RoleStatus.DELETED).getDisplayName()).toBe(
        '已删除',
      )
    })
  })
  ...
  describe('descriptions', () => {
    it('应该返回正确的描述', () => {
      expect(new RoleStatusValue(RoleStatus.ACTIVE).getDescription()).toBe(
        '角色处于激活状态，可以正常分配给用户',
      )
      expect(new RoleStatusValue(RoleStatus.SUSPENDED).getDescription()).toBe(
        '角色处于禁用状态，无法分配给新用户，但现有用户仍可使用',
      )
      expect(new RoleStatusValue(RoleStatus.DELETED).getDescription()).toBe(
        '角色已被删除，无法使用',
      )
    })
  })
