...
export const userInfrastructureConfig = registerAs(
  ...
  (): UserInfrastructureConfig => ({
    cache: {
      ...
      ttl: Number.parseInt(process.env.USER_CACHE_TTL || '3600', 10), // 1小时
      maxSize: Number.parseInt(process.env.USER_CACHE_MAX_SIZE || '1000', 10),
    },
    database: {
      connectionTimeout: Number.parseInt(
        process.env.USER_DB_CONNECTION_TIMEOUT || '5000',
        10,
      ),
      queryTimeout: Number.parseInt(
        process.env.USER_DB_QUERY_TIMEOUT || '30000',
        10,
      ),
      maxConnections: Number.parseInt(
        process.env.USER_DB_MAX_CONNECTIONS || '10',
        10,
      ),
    },
    external: {
      notification: {
        ...
        endpoint:
          process.env.USER_NOTIFICATION_ENDPOINT ||
          'http://localhost:3001/notifications',
        timeout: Number.parseInt(
          process.env.USER_NOTIFICATION_TIMEOUT || '5000',
          10,
        ),
      },
      email: {
        ...
        templatePath:
          process.env.USER_EMAIL_TEMPLATE_PATH || './templates/emails',
      },
    },
    security: {
      passwordMinLength: Number.parseInt(
        process.env.USER_PASSWORD_MIN_LENGTH || '8',
        10,
      ),
      passwordMaxLength: Number.parseInt(
        process.env.USER_PASSWORD_MAX_LENGTH || '128',
        10,
      ),
      passwordRequireSpecialChar:
        process.env.USER_PASSWORD_REQUIRE_SPECIAL_CHAR === 'true' || true,
      maxLoginAttempts: Number.parseInt(
        process.env.USER_MAX_LOGIN_ATTEMPTS || '5',
        10,
      ),
      lockoutDuration: Number.parseInt(
        process.env.USER_LOCKOUT_DURATION || '30',
        10,
      ), // 30分钟
      sessionTimeout: Number.parseInt(
        process.env.USER_SESSION_TIMEOUT || '1440',
        10,
      ), // 24小时
    },
  }),
