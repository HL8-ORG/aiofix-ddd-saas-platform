 ...
 * @description
 * 撤销会话命令处理器，实现CQRS模式中的命令处理部分。该处理器负责处理撤销会话命令，
 * 协调领域对象和服务完成具体的业务逻辑。
 *
 * 主要原理与机制：
...
import type { RevokeSessionUseCase } from '../use-cases/revoke-session.use-case'
...
export class RevokeSessionCommandHandler
  ...
  async execute(command: RevokeSessionCommand): Promise<RevokeSessionCommandResult> {
    try {
      // 记录命令执行开始
      this.logger.log(
        `开始执行撤销会话命令: ${JSON.stringify(command.toJSON())}`,
      ...
      // 记录命令执行成功
      this.logger.log(
        `撤销会话命令执行成功: sessionId=${command.sessionId}, reason=${command.reason}`,
        'RevokeSessionCommandHandler',
      )
  ...
  private validateCommand(command: RevokeSessionCommand): void {
    ...
    // 验证撤销者ID格式（如果提供）
    if (command.revokerId) {
      this.validateRevokerIdFormat(command.revokerId)
    }
  ...
  /**
   * @method validateRevokerIdFormat
   * @description 验证撤销者ID格式
   */
  private validateRevokerIdFormat(revokerId: string): void {
    const revokerIdRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
    if (!revokerIdRegex.test(revokerId)) {
      throw new Error('撤销者ID格式不正确')
    }
  }
