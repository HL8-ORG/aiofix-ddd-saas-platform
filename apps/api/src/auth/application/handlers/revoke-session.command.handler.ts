 ...
 * @description
 * 撤销会话命令处理器，实现CQRS模式中的命令处理逻辑。该处理器负责
 * 接收撤销会话命令，协调用例执行，并返回处理结果。
 *
 * 主要原理与机制：
...
export interface RevokeSessionCommandResult {
  success: boolean
  message?: string
  error?: string
  sessionId?: string
...
/**
 * @class RevokeSessionCommandHandler
 * @description 撤销会话命令处理器
 * @implements {ICommandHandler<RevokeSessionCommand, RevokeSessionCommandResult>}
 */
@Injectable()
@CommandHandler(RevokeSessionCommand)
export class RevokeSessionCommandHandler implements ICommandHandler<RevokeSessionCommand, RevokeSessionCommandResult> {
  constructor(
  ...
  /**
   * @method execute
   * @description 执行撤销会话命令
   * @param {RevokeSessionCommand} command - 撤销会话命令
   * @returns {Promise<RevokeSessionCommandResult>} 命令执行结果
   */
  async execute(command: RevokeSessionCommand): Promise<RevokeSessionCommandResult> {
    try {
      this.logger.log('开始处理撤销会话命令', {
      ...
      // 2. 执行用例
      const result = await this.revokeSessionUseCase.execute(command)
   ...
   * @param {RevokeSessionCommand} command - 撤销会话命令
   */
  private validateCommand(command: RevokeSessionCommand): void {
    // 1. 基础验证
    command.validate()
   ...
   * @param {RevokeSessionCommand} command - 撤销会话命令
   */
  private validateBusinessRules(command: RevokeSessionCommand): void {
    // 检查会话ID格式
    ...
    // 检查撤销原因
    this.validateRevokeReason(command.reason)
  ...
  /**
   * @method validateRevokeReason
   * @description 验证撤销原因
   * @param {string} reason - 撤销原因
   */
  private validateRevokeReason(reason: string): void {
    ...
    if (reason.length < 3 || reason.length > 200) {
      throw new Error('撤销原因长度必须在3到200个字符之间')
    }

    // 检查是否包含敏感词汇
    const sensitiveWords = ['script', 'javascript', 'eval', 'exec']
    const lowerReason = reason.toLowerCase()
    for (const word of sensitiveWords) {
      if (lowerReason.includes(word)) {
        throw new Error('撤销原因包含敏感词汇')
      }
    }
  }
   ...
   * @param {RevokeSessionCommand} command - 撤销会话命令
   * @param {RevokeSessionCommandResult} result - 执行结果
   */
  private logCommandExecution(command: RevokeSessionCommand, result: RevokeSessionCommandResult): void {
    if (result.success) {
      this.logger.log('撤销会话命令执行成功', {
        sessionId: command.sessionId,
        userId: command.userId,
        tenantId: command.tenantId,
        reason: command.reason,
