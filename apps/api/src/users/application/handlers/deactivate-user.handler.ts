 ...
 * @description
 * 停用用户命令处理器，实现CQRS模式中的命令处理部分。该处理器负责接收停用用户命令，
 * 并委托给对应的用例执行具体的业务逻辑。
 *
 * 主要原理与机制：
...
export class DeactivateUserHandler implements ICommandHandler<DeactivateUserCommand> {
  ...
  async execute(command: DeactivateUserCommand): Promise<DeactivateUserResult> {
    try {
      this.logger.log(
        `开始处理停用用户命令: commandId=${command.commandId}, userId=${command.userId}`,
      ...
      // 2. 委托给用例执行
      const result = await this.deactivateUserUseCase.execute(command)

      // 3. 记录处理结果
      this.logger.log(
        `停用用户命令处理完成: commandId=${command.commandId}, success=${result.success}`,
        'DeactivateUserHandler',
      )
      ...
      this.logger.error(
        `停用用户命令处理失败: commandId=${command.commandId}, error=${(error as Error).message}`,
