 ...
 * @description
 * 根据用户名查询用户用例，实现应用层业务逻辑。该用例负责协调领域对象和服务完成根据用户名查询用户的业务流程，
 * 包括权限验证、数据过滤、结果聚合等。
 *
 * 主要原理与机制：
...
export class GetUserByUsernameUseCase {
  ...
  async execute(query: GetUserByUsernameQuery): Promise<GetUserByUsernameResult> {
    try {
      this.logger.log(
        `开始执行根据用户名查询用户用例: username=${query.username}, tenantId=${query.tenantId}`,
      ...
      // 2. 查找用户
      const user = await this.findUserByUsername(query.username, query.tenantId)

      // 3. 检查用户状态
      if (!query.includeDeleted && user.getStatus().isDeleted()) {
        return {
          success: true,
          user: undefined,
        }
      ...
      this.logger.log(
        `根据用户名查询用户用例执行成功: username=${query.username}`,
        'GetUserByUsernameUseCase',
      )
      ...
      this.logger.error(
        `根据用户名查询用户用例执行失败: ${(error as Error).message}`,
   ...
   */
  private async findUserByUsername(username: string, tenantId: string): Promise<User> {
   ...
   * @param {User} user - 用户实体
   * @param {GetUserByUsernameQuery} query - 查询
   */
  private async auditUserQuery(user: User, query: GetUserByUsernameQuery): Promise<void> {
    this.logger.log(
      `用户查询审计: username=${query.username}, tenantId=${query.tenantId}, userId=${user.getId().getValue()}`,
      'GetUserByUsernameUseCase',
    )
  }
}
