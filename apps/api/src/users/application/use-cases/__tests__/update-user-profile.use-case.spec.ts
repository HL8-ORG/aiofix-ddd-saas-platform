...
describe('UpdateUserProfileUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('应该成功更新用户资料', async () => {
      ...
      // eventBus.publish 在 UpdateUserProfileUseCase 中被注释掉了，所以这里不期望被调用
      // expect(eventBus.publish).toHaveBeenCalled()
      expect(logger.log).toHaveBeenCalledWith(
        expect.stringContaining('更新用户资料用例执行成功'),
        'UpdateUserProfileUseCase'
      )
    })
    ...
    it('应该处理已删除用户的情况', async () => {
      ...
      // Assert
      expect(result.success).toBe(false)
      expect(result.error).toBe('已删除的用户无法更新资料')
      expect(logger.error).toHaveBeenCalled()
    })
    ...
    it('应该处理已锁定用户的情况', async () => {
      ...
      // Assert
      expect(result.success).toBe(false)
      expect(result.error).toBe('无法更新已锁定用户的资料')
      expect(logger.error).toHaveBeenCalled()
    })
    ...
    it('应该记录审计日志', async () => {
      ...
      // Assert
      expect(logger.log).toHaveBeenCalledWith(
        expect.stringContaining('开始执行更新用户资料用例'),
        'UpdateUserProfileUseCase'
      )
      expect(logger.log).toHaveBeenCalledWith(
        expect.stringContaining('更新用户资料用例执行成功'),
