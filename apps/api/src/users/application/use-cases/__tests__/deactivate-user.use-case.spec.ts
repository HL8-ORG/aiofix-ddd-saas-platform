...
describe('DeactivateUserUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('应该成功停用用户', async () => {
      ...
      expect(result.message).toBe('用户停用成功')
      ...
      // eventBus.publish 在 DeactivateUserUseCase 中被注释掉了，所以这里不期望被调用
      // expect(eventBus.publish).toHaveBeenCalled()
      expect(logger.log).toHaveBeenCalledWith(
        expect.stringContaining('停用用户用例执行成功'),
        'DeactivateUserUseCase'
      )
    })
    ...
    it('应该处理已删除用户的情况', async () => {
      ...
      // Assert
      expect(result.success).toBe(false)
      expect(result.error).toBe('已删除的用户无法停用')
      expect(logger.error).toHaveBeenCalled()
    })

    it('应该处理已停用用户的情况', async () => {
      // Arrange
      const command = createCommand()
      const inactiveUser = {
        ...mockUser,
      ...
      userRepository.findById.mockResolvedValue(inactiveUser as unknown as User)
      ...
      // Assert
      expect(result.success).toBe(false)
      expect(result.error).toBe('用户已经是非激活状态')
      expect(logger.error).toHaveBeenCalled()
    })
    ...
    it('应该记录审计日志', async () => {
      ...
      // Assert
      expect(logger.log).toHaveBeenCalledWith(
        expect.stringContaining('开始执行停用用户用例'),
        'DeactivateUserUseCase'
      )
      expect(logger.log).toHaveBeenCalledWith(
        expect.stringContaining('停用用户用例执行成功'),
