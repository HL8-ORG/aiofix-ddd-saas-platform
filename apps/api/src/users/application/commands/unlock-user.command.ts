 ...
 * @description
 * 解锁用户命令，实现CQRS模式中的命令部分。该命令封装了解锁用户所需的所有参数，
 * 并通过命令处理器执行具体的业务逻辑。
 *
 * 主要原理与机制：
...
export interface UnlockUserCommandDto {
  userId: string
  tenantId: string
  unlockedBy: string
  reason?: string
...
export class UnlockUserCommand {
   ...
   * @description 解锁者ID
   */
  @IsUUID('4', { message: '解锁者ID必须是有效的UUID v4格式' })
  ...
  /**
   * @constructor
   * @description 创建解锁用户命令
   * @param {UnlockUserCommandDto} data - 命令数据
   */
  constructor(data: UnlockUserCommandDto) {
  ...
  validate(): void {
    // 基础验证由装饰器处理
    // 业务规则验证
    if (this.userId === this.unlockedBy) {
      throw new Error('用户不能自己解锁自己')
    }
  }
   ...
   */
  toString(): string {
    return `UnlockUserCommand(userId=${this.userId}, tenantId=${this.tenantId})`
  }
}
