...
describe('identifier.util', () => {
  ...
  describe('normalizeIdentifier', () => {
    ...
    it('应该处理多个空格', () => {
      expect(normalizeIdentifier('test   tenant')).toBe('test---tenant');
      expect(normalizeIdentifier('test  tenant  code')).toBe('test--tenant--code');
    });
  ...
  describe('validateIdentifier', () => {
    ...
    it('应该拒绝包含大写字母的编码', () => {
      expect(() => validateIdentifier('Test-tenant')).toThrow('编码只能包含小写字母、数字和连字符');
      expect(() => validateIdentifier('test-Tenant')).toThrow('编码只能包含小写字母、数字和连字符');
    });
    ...
    it('应该拒绝包含特殊字符的编码', () => {
      ...
      invalidChars.forEach(char => {
        expect(() => validateIdentifier(`test${char}tenant`)).toThrow('编码只能包含小写字母、数字和连字符');
      });
    });

    it('应该拒绝以连字符开头或结尾的编码', () => {
      expect(() => validateIdentifier('-test-tenant')).toThrow('编码不能以连字符开头或结尾');
      expect(() => validateIdentifier('test-tenant-')).toThrow('编码不能以连字符开头或结尾');
      expect(() => validateIdentifier('-test-tenant-')).toThrow('编码不能以连字符开头或结尾');
    });
    ...
    it('应该根据配置处理连续连字符', () => {
      ...
      // 不允许连续连字符
      expect(() => validateIdentifier('test--tenant', { allowConsecutiveHyphens: false })).toThrow('编码不能包含连续的连字符');
    });

    it('应该支持自定义长度范围', () => {
      // 自定义最小长度
      expect(() => validateIdentifier('ab', { minLength: 2 })).not.toThrow();
      expect(() => validateIdentifier('a', { minLength: 2 })).toThrow('编码至少需要2个字符');

      // 自定义最大长度
      expect(() => validateIdentifier('abc', { maxLength: 3 })).not.toThrow();
      expect(() => validateIdentifier('abcd', { maxLength: 3 })).toThrow('编码不能超过3个字符');
    });
    ...
    it('应该处理边界值', () => {
      ...
      expect(() => validateIdentifier(overMaxCode)).toThrow('编码不能超过50个字符');
    });
  });
  ...
  describe('集成测试', () => {
    ...
    it('应该处理无效输入', () => {
      ...
      invalidInputs.forEach(input => {
        const normalized = normalizeIdentifier(input);
        expect(() => validateIdentifier(normalized)).toThrow('编码只能包含小写字母、数字和连字符');
      });
    });
  });
