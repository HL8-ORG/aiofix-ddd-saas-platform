...
describe('租户领域事件', () => {
  ...
  describe('TenantCreatedEvent', () => {
    it('应该创建租户创建事件', () => {
      ...
      expect(event.tenantData.settings).toEqual({ theme: 'dark', language: 'zh-CN' });
    });
  });
  ...
  describe('TenantSettingsUpdatedEvent', () => {
    it('应该创建租户配置更新事件', () => {
      ...
      const newSettings = { theme: 'dark', language: 'zh-CN', notifications: true };
      const updatedBy = generateUuid();
      const event = new TenantSettingsUpdatedEvent(tenant, oldSettings, newSettings, updatedBy);
