...
describe('租户基础设施层数据库连接测试', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      tenantRepository = module.get<TenantRepositoryMikroOrm>('TenantRepository');
  ...
  describe('MikroORM仓储数据库操作测试', () => {
    ...
    it('应该能够从数据库查询租户', async () => {
      ...
      const foundByCode = await tenantRepository.findByCodeString('query-test-tenant');
    ...
    it('应该能够更新租户状态', async () => {
      ...
      const updated = await tenantRepository.updateStatus(tenant.id, TenantStatus.ACTIVE);
