...
describe('租户基础设施层单元测试', () => {
  ...
  describe('仓储实现', () => {
    ...
    it('应该能够检查租户是否存在', async () => {
      ...
      expect(await tenantRepository.existsByCodeString('test-tenant')).toBe(
        false,
      )
    ...
    it('应该能够统计租户数量', async () => {
      ...
      const pendingCount = await tenantRepository.countByStatus(
  ...
  describe('通知服务', () => {
    ...
    it('应该能够发送激活通知', async () => {
      ...
      await notificationService.sendActivationNotification(
        tenant,
        'admin-user-id',
      )
