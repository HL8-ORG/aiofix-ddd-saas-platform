...
describe('用户基础设施层数据库连接测试', () => {
  ...
  describe('MikroORM仓储数据库操作测试', () => {
    ...
    it('应该能够从数据库查询用户', async () => {
      ...
      const foundByUsername = await userRepository.findByUsernameString('query-test-user', user.tenantId);
      expect(foundByUsername).toBeDefined();
      expect(foundByUsername?.getUsername()).toBe('query-test-user');

      const foundByEmail = await userRepository.findByEmailString('query-test@example.com', user.tenantId);
    ...
    it('应该能够更新用户状态', async () => {
      ...
      // 激活用户
      user.activate();
      await userRepository.updateStatus(user.id, UserStatusValue.active(), user.tenantId);
      ...
      // 禁用用户
      user.suspend();
      await userRepository.updateStatus(user.id, UserStatusValue.suspended(), user.tenantId);
