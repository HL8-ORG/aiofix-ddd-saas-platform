...
export class UserRepositoryMemory extends UserRepository {
  ...
  async save(user: User): Promise<User> {
    // 检查用户名重复
    const existingUserByUsername = await this.findByUsernameString(
      user.getUsername(),
      user.tenantId,
    )
    ...
    // 检查邮箱重复
    const existingUserByEmail = await this.findByEmailString(
      user.getEmail(),
      user.tenantId,
    )
  ...
  async findById(id: string, tenantId: string): Promise<User | null> {
    ...
    // 只返回未删除的用户
    return user && user.tenantId === tenantId && user.getStatus() !== 'DELETED'
      ? user
      : null
  }
  ...
  async findByStatus(
    ...
    for (const user of this.users.values()) {
      if (
        user.tenantId === tenantId &&
        user.getStatus() === status.toString()
      ) {
        users.push(user)
      }
    }
  ...
  async findWithPagination(
    ...
    for (const user of this.users.values()) {
      ...
      if (
        ...
        !user.organizationIds?.includes(filters.organizationId)
      ) {
        continue
      }
      ...
      if (filters?.search) {
        ...
        const matches =
          user.getUsername().toLowerCase().includes(search) ||
  ...
  async findBySearch(
    ...
    for (const user of this.users.values()) {
      if (
        ...
        const matches =
          user.getUsername().toLowerCase().includes(searchLower) ||
