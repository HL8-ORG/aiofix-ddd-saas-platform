...
describe('Email', () => {
  ...
  describe('无效邮箱地址', () => {
    ...
    it('应该拒绝本地部分过长的邮箱地址', () => {
      ...
      expect(() => new Email(longLocalPart)).toThrow('邮箱地址本地部分长度不能超过64个字符');
    });

    it('应该拒绝域名部分过长的邮箱地址', () => {
      const longDomainPart = 'user@' + 'a'.repeat(254) + '.com';
      expect(() => new Email(longDomainPart)).toThrow('邮箱地址长度不能超过254个字符');
    });
  ...
  describe('国际化邮箱地址', () => {
    ...
    it('应该支持包含特殊字符的域名', () => {
      expect(() => new Email('user@example-domain.com')).not.toThrow();
      expect(() => new Email('user@example_domain.com')).toThrow('邮箱地址格式无效');
    });
  });
}); 