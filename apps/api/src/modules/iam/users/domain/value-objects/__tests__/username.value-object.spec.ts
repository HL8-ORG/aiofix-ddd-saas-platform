...
describe('Username', () => {
  ...
  describe('无效用户名', () => {
    ...
    it('应该拒绝过长的用户名', () => {
      ...
      expect(() => new Username(longUsername)).toThrow('用户名长度不能超过50个字符');
    });

    it('应该拒绝以数字开头的用户名', () => {
      expect(() => new Username('123user')).toThrow('用户名格式无效：必须以字母开头，只能包含字母、数字、下划线、连字符，不能以特殊字符结尾');
      expect(() => new Username('1user')).toThrow('用户名格式无效：必须以字母开头，只能包含字母、数字、下划线、连字符，不能以特殊字符结尾');
    });
    ...
    it('应该拒绝包含无效字符的用户名', () => {
      ...
      invalidUsernames.forEach(username => {
        expect(() => new Username(username)).toThrow('用户名格式无效：必须以字母开头，只能包含字母、数字、下划线、连字符，不能以特殊字符结尾');
      });
    });

    it('应该拒绝包含连续特殊字符的用户名', () => {
      expect(() => new Username('user__name')).toThrow('用户名不能包含连续的特殊字符');
      expect(() => new Username('user--name')).toThrow('用户名不能包含连续的特殊字符');
      expect(() => new Username('user_-name')).toThrow('用户名不能包含连续的特殊字符');
      expect(() => new Username('user-_name')).toThrow('用户名不能包含连续的特殊字符');
    });

    it('应该拒绝以特殊字符结尾的用户名', () => {
      expect(() => new Username('username_')).toThrow('用户名不能以特殊字符结尾');
      expect(() => new Username('username-')).toThrow('用户名不能以特殊字符结尾');
    });
  });
