import { UserStatusValue, UserStatus } from '../user-status.value-object';
...
describe('UserStatusValue Value Object', () => {
  ...
  describe('构造函数', () => {
    ...
    it('应该拒绝无效的用户状态', () => {
      expect(() => new UserStatusValue('INVALID' as any)).toThrow('无效的用户状态');
  ...
  describe('显示信息', () => {
    it('应该返回正确的显示名称', () => {
      expect(new UserStatusValue(UserStatus.PENDING).getDisplayName()).toBe('待激活');
      expect(new UserStatusValue(UserStatus.ACTIVE).getDisplayName()).toBe('激活');
      expect(new UserStatusValue(UserStatus.SUSPENDED).getDisplayName()).toBe('禁用');
      expect(new UserStatusValue(UserStatus.DELETED).getDisplayName()).toBe('已删除');
    });

    it('应该返回正确的描述', () => {
      expect(new UserStatusValue(UserStatus.PENDING).getDescription()).toBe('用户已注册但尚未激活，需要管理员激活或邮箱验证');
      expect(new UserStatusValue(UserStatus.ACTIVE).getDescription()).toBe('用户已激活，可以正常登录和使用系统');
      expect(new UserStatusValue(UserStatus.SUSPENDED).getDescription()).toBe('用户已被禁用，无法登录系统');
      expect(new UserStatusValue(UserStatus.DELETED).getDescription()).toBe('用户已被删除，数据保留用于审计');
    });
  });
  ...
  describe('边界情况', () => {
    ...
    it('应该处理大小写敏感', () => {
      ...
      expect(() => new UserStatusValue('active' as any)).toThrow('无效的用户状态');
      expect(() => new UserStatusValue(UserStatus.ACTIVE)).not.toThrow();
    });
  });
}); 