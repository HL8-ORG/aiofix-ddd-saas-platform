...
describe('RolePriority', () => {
  ...
  describe('validation', () => {
    ...
    it('应该拒绝null值', () => {
      expect(() => new RolePriority(null as any)).toThrow('角色优先级不能大于1000, 角色优先级不能小于1, 角色优先级必须是数字');
    });

    it('应该拒绝undefined值', () => {
      expect(() => new RolePriority(undefined as any)).toThrow('角色优先级不能大于1000, 角色优先级不能小于1, 角色优先级必须是数字');
    });

    it('应该拒绝非数字值', () => {
      expect(() => new RolePriority('invalid' as any)).toThrow('角色优先级不能大于1000, 角色优先级不能小于1, 角色优先级必须是数字');
    });
  ...
  describe('methods', () => {
    ...
    it('getDescription应该返回描述', () => {
      const priority = new RolePriority(1);
      expect(priority.getDescription()).toBe('系统级角色，拥有最高权限，可管理所有租户');
    });
  ...
  describe('descriptions', () => {
    it('应该返回正确的描述', () => {
      expect(new RolePriority(1).getDescription()).toBe('系统级角色，拥有最高权限，可管理所有租户');
      expect(new RolePriority(10).getDescription()).toBe('租户级角色，拥有租户内最高权限，可管理租户内所有资源');
      expect(new RolePriority(50).getDescription()).toBe('组织级角色，拥有组织内管理权限，可管理组织内资源');
      expect(new RolePriority(100).getDescription()).toBe('用户级角色，拥有基本操作权限，可进行日常业务操作');
      expect(new RolePriority(200).getDescription()).toBe('访客级角色，拥有只读权限，仅可查看部分信息');
    });
  });
