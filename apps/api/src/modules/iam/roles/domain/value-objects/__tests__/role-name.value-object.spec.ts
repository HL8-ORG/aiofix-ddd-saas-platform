...
describe('RoleName', () => {
  ...
  describe('验证规则', () => {
    ...
    it('应该拒绝超过50个字符的名称', () => {
      ...
      expect(() => new RoleName(longName)).toThrow(
        '角色名称长度不能超过50个字符',
      )
    })
    ...
    it('应该拒绝包含连续特殊字符的名称', () => {
      expect(() => new RoleName('Admin@@User')).toThrow(
        '角色名称不能包含连续的特殊字符',
      )
    })
  ...
  describe('方法测试', () => {
    ...
    it('getShortName应该截断长名称', () => {
      const longRoleName = new RoleName(
        '这是一个非常长的角色名称用于测试截断功能',
      )
