...
describe('Permission', () => {
  ...
  describe('conditions management', () => {
    ...
    it('应该拒绝为不支持条件的权限类型设置条件', () => {
      ...
      expect(() => menuPermission.setConditions(conditions)).toThrow(
        '当前权限类型不支持条件设置',
      )
    })
  })
  ...
  describe('fields management', () => {
    ...
    it('应该拒绝为不支持字段的权限类型设置字段', () => {
      ...
      expect(() => menuPermission.setFields(['id'])).toThrow(
        '当前权限类型不支持字段设置',
      )
    })
  })
  ...
  describe('getter methods', () => {
    ...
    it('应该返回正确的类型信息', () => {
      ...
      expect(permission.getTypeDescription()).toBe(
        '控制用户对后端接口的调用权限',
      )
    })
    ...
    it('应该返回正确的操作信息', () => {
      ...
      expect(permission.getActionDescription()).toBe(
        '允许完全管理资源（包含所有操作）',
      )
    })
  })
