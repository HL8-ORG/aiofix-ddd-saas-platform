import { PermissionCondition, PermissionConditionData } from '../permission-condition.value-object';
...
describe('PermissionCondition', () => {
  describe('constructor', () => {
    ...
    it('应该成功创建多个条件', () => {
      const conditionData: PermissionConditionData[] = [
        ...
        { field: 'type', operator: 'in', value: ['user', 'admin'], logicalOperator: 'and' }
      ];
      const condition = new PermissionCondition(conditionData);
      expect(condition.getValue()).toEqual(conditionData);
    });
  ...
  describe('validation', () => {
    it('应该拒绝非数组输入', () => {
      expect(() => new PermissionCondition(null as any)).toThrow('权限条件必须是数组格式');
    });
    ...
    it('应该拒绝包含无效字段的条件', () => {
      ...
      expect(() => new PermissionCondition([invalidCondition])).toThrow('权限条件字段不能为空且必须是字符串');
    });
    ...
    it('应该拒绝包含无效操作符的条件', () => {
      ...
      expect(() => new PermissionCondition([invalidCondition])).toThrow('无效的操作符: invalid');
    });
    ...
    it('应该拒绝包含空值的条件', () => {
      ...
      expect(() => new PermissionCondition([invalidCondition])).toThrow('权限条件值不能为空');
    });
    ...
    it('应该拒绝包含无效逻辑运算符的条件', () => {
      ...
      expect(() => new PermissionCondition([invalidCondition])).toThrow('无效的逻辑运算符: invalid');
    });
  });
  ...
  describe('toCaslCondition', () => {
    ...
    it('应该转换包含OR逻辑运算符的条件', () => {
      const condition = new PermissionCondition([
        { field: 'status', operator: 'eq', value: 'active' },
        { field: 'type', operator: 'in', value: ['user', 'admin'], logicalOperator: 'or' }
      ]);
      const caslCondition = condition.toCaslCondition();
      expect(caslCondition).toEqual({
        $or: [
  ...
  describe('static factory methods', () => {
    ...
    it('应该通过静态方法创建简单条件', () => {
      const condition = PermissionCondition.createSimple('status', 'eq', 'active');
      expect(condition.getValue()).toEqual([
        { field: 'status', operator: 'eq', value: 'active' }
      ]);
    });
