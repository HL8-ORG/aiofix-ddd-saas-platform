...
describe('PermissionCode', () => {
  ...
  describe('validation', () => {
    ...
    it('应该拒绝undefined值', () => {
      expect(() => new PermissionCode(undefined as any)).toThrow('权限代码不能为空');
    });

    it('应该拒绝少于3个字符的代码', () => {
      expect(() => new PermissionCode('AB')).toThrow('权限代码长度不能少于3个字符');
    });

    it('应该拒绝超过30个字符的代码', () => {
      const longCode = 'A'.repeat(31);
      expect(() => new PermissionCode(longCode)).toThrow('权限代码长度不能超过30个字符');
    });

    it('应该拒绝不以字母开头的代码', () => {
      expect(() => new PermissionCode('1USER_MANAGE')).toThrow('权限代码必须以字母开头');
    });
