...
describe('PermissionName', () => {
  ...
  describe('validation', () => {
    ...
    it('应该拒绝undefined值', () => {
      expect(() => new PermissionName(undefined as any)).toThrow('权限名称不能为空');
    });

    it('应该拒绝少于2个字符的名称', () => {
      expect(() => new PermissionName('A')).toThrow('权限名称长度不能少于2个字符');
    });

    it('应该拒绝超过50个字符的名称', () => {
      const longName = 'A'.repeat(51);
      expect(() => new PermissionName(longName)).toThrow('权限名称长度不能超过50个字符');
    });

    it('应该拒绝以数字开头的名称', () => {
      expect(() => new PermissionName('1用户管理')).toThrow('权限名称不能以数字开头');
    });

    it('应该拒绝包含连续特殊字符的名称', () => {
      expect(() => new PermissionName('用户@@管理')).toThrow('权限名称不能包含连续的特殊字符');
    });

    it('应该拒绝包含不允许字符的名称', () => {
      expect(() => new PermissionName('用户管理@')).toThrow('权限名称只能包含字母、数字、下划线、连字符、中文和空格');
    });
  ...
  describe('getShortName', () => {
    ...
    it('应该返回截断的短名称（当长度超过20个字符时）', () => {
      ...
      expect(permissionName.getShortName()).toBe('这是一个非常非常长的权限名称用于测试截断...');
    });
  });
