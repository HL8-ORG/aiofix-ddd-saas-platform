import { PermissionTypeValue, PermissionType } from '../permission-type.value-object';
...
describe('PermissionTypeValue', () => {
  ...
  describe('validation', () => {
    it('应该拒绝无效的权限类型', () => {
      expect(() => new PermissionTypeValue('invalid' as any)).toThrow('无效的权限类型: invalid');
    });
  });
  ...
  describe('getDescription', () => {
    it('应该返回菜单权限的描述', () => {
      const permissionType = new PermissionTypeValue(PermissionType.MENU);
      expect(permissionType.getDescription()).toBe('控制用户对系统菜单的访问权限');
    });

    it('应该返回按钮权限的描述', () => {
      const permissionType = new PermissionTypeValue(PermissionType.BUTTON);
      expect(permissionType.getDescription()).toBe('控制用户对页面按钮的操作权限');
    });

    it('应该返回接口权限的描述', () => {
      const permissionType = new PermissionTypeValue(PermissionType.API);
      expect(permissionType.getDescription()).toBe('控制用户对后端接口的调用权限');
    });

    it('应该返回数据权限的描述', () => {
      const permissionType = new PermissionTypeValue(PermissionType.DATA);
      expect(permissionType.getDescription()).toBe('控制用户对数据的访问和操作权限');
    });
  });
