import { PermissionStatusValue, PermissionStatus } from '../permission-status.value-object';

describe('PermissionStatusValue', () => {
  describe('constructor', () => {
    it('应该成功创建激活状态', () => {
      const permissionStatus = new PermissionStatusValue(PermissionStatus.ACTIVE);
      expect(permissionStatus.getValue()).toBe(PermissionStatus.ACTIVE);
    });

    it('应该成功创建禁用状态', () => {
      const permissionStatus = new PermissionStatusValue(PermissionStatus.INACTIVE);
      expect(permissionStatus.getValue()).toBe(PermissionStatus.INACTIVE);
    });

    it('应该成功创建暂停状态', () => {
      const permissionStatus = new PermissionStatusValue(PermissionStatus.SUSPENDED);
      expect(permissionStatus.getValue()).toBe(PermissionStatus.SUSPENDED);
    });

    it('应该成功创建过期状态', () => {
      const permissionStatus = new PermissionStatusValue(PermissionStatus.EXPIRED);
  ...
  describe('validation', () => {
    it('应该拒绝无效的权限状态', () => {
      expect(() => new PermissionStatusValue('invalid' as any)).toThrow('无效的权限状态: invalid');
    });
  });
  ...
  describe('getDescription', () => {
    ...
    it('应该返回暂停状态的描述', () => {
      const permissionStatus = new PermissionStatusValue(PermissionStatus.SUSPENDED);
      expect(permissionStatus.getDescription()).toBe('权限已暂停，需要管理员重新启用');
    });
